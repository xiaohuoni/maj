...
export class ModelUtils {
  ...
  opts: IOpts = {};
  count: number = 1;
  constructor(paths: Paths | null, opts: IOpts) {
    this.paths = paths as Paths;
    this.opts = opts;
  }
  ...
  getAllModels(opts: { sort?: object; extraModels: string[] }) {
    ...
    const models = [
      ...this.getModels({
        base: join(this.paths.absSrcPath, 'models'),
        pattern: '**/*.{ts,tsx,js,jsx}',
      }),
      ...this.getModels({
        base: join(this.paths.absPagesPath),
      ...
      return new Model(file, this.paths.absSrcPath, opts.sort, this.count++);
    });
    // check duplicate
...
export async function getAllModels(paths: Paths, extraModels: string[] = []) {
  return new ModelUtils(paths, {
    astTest({ node }) {
